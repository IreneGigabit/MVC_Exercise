...
app.directive('roomType', function () {//須駝峰式命名法.tag名則用-分開(<room-type></room-type>)
    return {
        ...
        template: '<select name="selRoomType" id="selRoomType" required ng-click="test()">' +
                '<option value="" selected>-- Select --</option>'+
                ' <option ng-repeat="t in vm.roomTypeData" value="{{t.VLS_CODE}}">{{t.VLS_CODE}}_{{t.VLS_NAME}}{{t.ATTRIBUTE1|myFilter}}</option>' +
                '</select>',

        // 當使用了bindToController選項後，組件的屬性會被綁定到controller上，而不是scope上，
        // 並且我們可以把原本定義在scope中的屬性綁定都遷移到bindToController選項中來，並成為對象字面量的形式
        bindToController: {
            roomTypeData: '='//,
            //onAddressClicked: '&' // Address被點擊時需要呼叫的外部方法 
        },
